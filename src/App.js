...
function AddActivityForm({ onAdd, onClose }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="form-container">
      ...
      />
      <button type="submit" className="button-style">Tambah</button>
    </form>
  );
}
...
function TaskCard({ task, picName, onPauseToggle, onMoveLeft, onMoveRight }) {
  ...
  useEffect(() => {
    if (isPaused) {
      pauseStartRef.current = pauseStartRef.current || new Date(task.pause_time).getTime();
  ...
  const togglePause = () => {
    if (isPaused) {
      ...
      const pauseDuration = Math.floor((pauseEnd - pauseStartRef.current) / 60000);
  ...
  const totalPause = (task.minute_pause || 0) + (isPaused ? currentPauseMinutes : 0);
  ...
  return (
    ...
    >
      <div><b>Content:</b> {task.content}</div>
      <div><b>Detail:</b> {task.detail}</div>
      <div style={{ marginTop: 8 }}>
        <button onClick={() => onMoveLeft(task)} disabled={task.status === "todo"}>
          ←
        </button>
        <button onClick={() => onMoveRight(task)} disabled={task.status === "archived"} style={{ marginLeft: 8 }}>
          →
        </button>
      ...
      <div style={{fontSize: 12, display: 'grid',
        gap: '6px 20px',
        marginTop: 10 }}>
        <small>
        <div>To Do: {task.timestamp_todo ? new Date(task.timestamp_todo).toLocaleString() : "-"}, Progress: {task.timestamp_progress ? new Date(task.timestamp_progress).toLocaleString() : "-"}</div>
        <div>Done: {task.timestamp_done ? new Date(task.timestamp_done).toLocaleString() : "-"}, Archived: {task.timestamp_archived ? new Date(task.timestamp_archived).toLocaleString() : "-"}</div>
        <div>Pause Started At: {task.pause_time ? new Date(task.pause_time).toLocaleString() : "-"}, Pause Minutes: {totalPause}</div>
        </small>
      </div>
      {task.status === "done" && (
        <div style ={{marginTop: 10}}><b>Activity Minutes:</b> {task.minute_activity || 0}</div>
      )}
...
function KanbanColumn({ status, tasks, pics, onMoveLeft, onMoveRight, onPauseToggle }) {
  return (
    <div
      style={{
        flex: 1,
...
export default function App() {
  ...
  const fetchActivities = async () => {
    try {
      const res = await fetch("http://192.168.1.14:8080/api/activities");
  ...
  const fetchPics = async () => {
    try {
      const res = await fetch("http://192.168.1.14:8080/api/pics");
      const data = await res.json();
      setPics(data);
    } catch (err) {
  ...
  const fetchTasks = async () => {
    try {
      const res = await fetch("http://192.168.1.14:8080/api/tasks");
      const data = await res.json();
      setTasks(data);
    } catch (err) {
  ...
  const updateTaskStatus = async (taskId, newStatus) => {
    try {
      ...
      const res = await fetch(`http://192.168.1.14:8080/api/tasks/${taskId}/status`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          status: newStatus,
   ...
   */
  const onPauseToggle = async (taskId, pauseMinutes, resetPauseTime = false, newPauseTime = null) => {
    try {
      const task = tasks.find((t) => t.id === taskId);
      if (!task) return;
      ...
      const res = await fetch(`http://192.168.1.14:8080/api/tasks/${taskId}/pause`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      setTasks((prev) =>
        prev.map((t) =>
          t.id === taskId
            ? { ...t, minute_pause: updatedMinutePause, pause_time: updatedPauseTime }
            : t
        )
      );
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <footer
        style={{ textAlign: "center", padding: 10, backgroundColor: "#001529", color: "white" }}
      >
